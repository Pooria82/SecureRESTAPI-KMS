...
async def login_user(user: UserLogin, db=Depends(get_db)):
    ...
    failed_attempts = db_user.get("failed_attempts", 0)
    last_failed_attempt_time = db_user.get("last_failed_attempt_time")
    if failed_attempts >= 5:
        if last_failed_attempt_time is not None:
            time_since_last_attempt = (datetime.utcnow() - last_failed_attempt_time).total_seconds()
            if time_since_last_attempt < 900:
                raise HTTPException(status_code=400, detail="Too many failed attempts. Try again later.")
        else:
            # If last_failed_attempt_time is None or not present, reset failed_attempts
    ...
    if not pwd_context.verify(user.password + db_user["salt"] + settings.pepper, db_user["hashed_password"]):
        await user_collection.update_one({"_id": db_user["_id"]}, {"$inc": {"failed_attempts": 1}, "$set": {"last_failed_attempt_time": datetime.utcnow()}})
        raise HTTPException(status_code=400, detail="Invalid username or password")
    ...
    token = jwt.encode({"sub": user.username, "exp": datetime.utcnow() + timedelta(hours=1)}, settings.jwt_secret, algorithm="HS256")
...
async def send_otp(otp_request: OTPRequest, db=Depends(get_db)):
    ...
    totp_secret = db_user.get("totp_secret")
    if totp_secret is None:
        raise HTTPException(status_code=400, detail="TOTP secret not set for this user")
...
async def verify_otp(otp_verify: OTPVerify, db=Depends(get_db)):
    ...
    token = jwt.encode({"sub": otp_verify.username, "exp": datetime.utcnow() + timedelta(hours=1)}, settings.jwt_secret, algorithm="HS256")
    return {"access_token": token, "token_type": "bearer"}